...
public class RecetaController : ControllerBase
{
    ...
    [HttpPost("{id}/ingredientes", Name = "AsignarIngredientesReceta")]
    public async Task<ActionResult> AsignarIngredienteReceta(int id, [FromBody] RecetaIngrediente dto)
    {
        try
        {
            var resultado = await gestionarRecetasBW.AregarIngredienteReceta(id, dto);
            if (!resultado)
                return NotFound("No se pudo asignar el ingrediente a la receta.");
            return Ok();
        }
        catch (Exception ex)
        {
    ...
    [HttpPut("{id}/ingredientes/{ingredienteId}", Name = "ActualizarIngredienteReceta")]
    public async Task<ActionResult> ActualizarIngredienteReceta(int id, int ingredienteId, [FromBody] RecetaIngrediente dto)
    {
        try
        {
            var resultado = await gestionarRecetasBW.ActualizarIngredienteReceta(id, ingredienteId, dto);
            if (!resultado)
                return NotFound("Relación receta-ingrediente no encontrada.");
            return Ok();
        }
        catch (Exception ex)
        {
    ...
    [HttpDelete("{id}/ingredientes/{ingredienteId}", Name = "EliminarIngredienteReceta")]
    public async Task<ActionResult> EliminarIngredienteReceta(int id, int ingredienteId)
    {
        try
        {
            var resultado = await gestionarRecetasBW.EliminarIngredienteReceta(id, ingredienteId);
            if (!resultado)
                return NotFound("Relación receta-ingrediente no encontrada.");
            return Ok();
        }
